...
var CBusCore = function () {
  function CBusCore(cmdParse) {
    _classCallCheck(this, CBusCore);
  ...
  _createClass(CBusCore, [{
    ...
    key: 'addProtoBuilder',
    value: function addProtoBuilder(protoFileName, requireObj) {
      try {
    ...
    value: function buildProtoPackage(proto_package) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (_this.PROTO_FILE_DIR[_this.PROTO_FILE_DIR.length - 1] !== '/') {
          _this.PROTO_FILE_DIR += '/';
        }
        var protoFilePath = _this.PROTO_FILE_DIR + proto_package + ".proto";
        ProtoBuf.load(protoFilePath).then(function (root) {
    ...
    value: function buildProtoObject(proto_package, proto_objectname) {
      var _this2 = this;

      return new Promise(function (resolve, reject) {
        var packageName = proto_package;
        var objectName = proto_objectname;
        return _this2.buildProtoPackage(packageName).then(function (root) {
          var obj = root.lookupTypeOrEnum(objectName);
          if (obj) {
            // console.log('buildProtoObject', proto_package, proto_objectname)
            return resolve(obj);
          }
          var errStr = 'builerProtoObject ' + objectName + ' failed';
    ...
    key: 'requestOnce',
    value: function requestOnce(cmd, proto_package, proto_request, callback) {
      var _this3 = this;

      return this.buildProtoObject(proto_package, proto_request).then(function (obj) {
        var payload = {};
      ...
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        if (forever === undefined || !forever) {
          ...
          this.subscribeInfo(PREFIX_DATABUS, serialnum, function (info, command) {
            var protoName = self.cmdParse.getProtoFileName(command);
            var className = self.cmdParse.getClassName(command);
            if (protoName.length && className.length) {
              if (protoName === 'msgexpress' && className === 'MsgExpress.ErrMessage') {
                self.buildProtoObject(protoName, className).then(function (obj) {
                  try {
                    var msg = obj.decode(info.view);
